...
while(true){
    ...
    if (inp>num){
        ...
        console.log(`well played ${name}, congratulations!!! you guessed it right in ${count} times`)
        break;
    }
    res++;
} 
...
const movies = [
  ...
  },
  {
    title: "Lagaan",
    hints: [
      "It starts with 'L'. it was a very long movie of amir khan",
...
function startGame() {
  console.log("Welcome to the Dumb charades Game!");
  ...
  console.log("Guess the movie based on the following hints:");

  // Main game loop
  while (hintIndex < movie.hints.length) {
    console.log(`Hint ${hintIndex + 1}: ${movie.hints[hintIndex]}`);
    const userGuess = prompt("Your guess: ").trim().toLowerCase();

    if (userGuess === movie.title.toLowerCase()) {
      console.log(`Correct! The movie is "${movie.title}".`);
      ...
      if (hintIndex === movie.hints.length) {
        console.log("Sorry, you've used all your hints.");
        console.log(`The correct answer was "${movie.title}".`);
      } else {
        console.log("Incorrect guess, here's another hint.");
      }
    }
  }
